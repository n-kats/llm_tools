...
import json
from typing import TypeVar, Type, Generic, Callable, Any, Optional
import traceback
...
class Adapter(Generic[T_CONTEXT, T_IN]):
    ...
    @classmethod
    def identity(cls, type_: Type[T_IN]) -> "Adapter[T_IN, T_IN]":
        def fn(x: T_IN) -> T_IN:
            return x
        return cls(fn)
...
class Executor(ABC, Generic[T_IN]):
    ...
    def build_function(
        self, name: str, adapter: Adapter, typed_prompt: TypedPrompt, next_type: Type[BaseModel]
    ) -> Callable[[ExecutionState], tuple[Any, ExecutionState]]:
        return self._wrap_base_function(self._build_base_function(
            name, adapter, typed_prompt, next_type
        ))
    ...
    def _build_base_function(
        ...
        def fn(context: BaseModel) -> Any:
            ...
            result = typed_prompt.parse(
                self.__fn(typed_prompt.generate_input(input_)))
...
class TacticBuilder:
    ...
    def add_typed_prompt(self, name: str, adapter: Adapter, typed_prompt: TypedPrompt, executor: Executor):
        ...
        self.__functions.append(
                executor.build_function(
                    name, adapter, typed_prompt, next_context_type))
